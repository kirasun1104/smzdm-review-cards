...
- On `hero-blue` cover pages, lower metric / CTA bands that sit over the pale end of the blue gradient keep the route's transparent/open rule treatment and use black or dark ink for main values. Do not add a filled background, tint plate, overlay, or shadow to those bands merely to satisfy contrast validation; if validation misreads the gradient background, fix the validator or the shared route rule narrowly.
